...
export default {
    ...
    methods: {
        ...
        validate(callback) {
            ...
            if (!utils.isEmpty(this.userName) && !utils.isEmpty(this.password) && !utils.isEmpty(this.checkCode)) {
                callback(true)
            } else {
                callback(false)
            }
...
.login-header {
    height: 90px;
    width: 100%;
    background: rgba(0, 0, 0, 0.38) url(/static/images/login-logo.png) no-repeat
        center;
}
.login-content {
    padding: 35px 55px;
